...
static	inline	unsigned long long	wyhash_mul_mod(unsigned long long a, unsigned long long b, unsigned long long m) {
...
static inline unsigned long long wyhash_pow_mod(unsigned long long a, unsigned long long b, unsigned long long m) {
    unsigned long long r=1;
    while (b) {
        if (b&1) r=wyhash_mul_mod(r,a,m);
        b>>=1;
        if (b) a=wyhash_mul_mod(a,a,m);
    }
    return r;
}
unsigned wyhash_sprp(unsigned long long n, unsigned long long a) {
    ...
    if (!(d & 0x3)) { d>>=2; s+=2; }
    if (!(d & 0x1)) { d>>=1; s+=1; }
    unsigned long long b=wyhash_pow_mod(a,d,n);
    if ((b==1) || (b==(n-1))) return 1;
    unsigned char r;
    for (r=1; r<s; r++) {
        b=wyhash_mul_mod(b,b,n);
        if (b<=1) return 0;
        if (b==(n-1)) return 1;
    }
    return 0;
}
unsigned wyhash_is_prime(unsigned long long n) {
    if (n<2||!(n&1)) return 0;
    if (n<4) return 1;
    if (!wyhash_sprp(n,2)) return 0;
    if (n<2047) return 1;
    if (!wyhash_sprp(n,3)) return 0;
    if (!wyhash_sprp(n,5)) return 0;
    if (!wyhash_sprp(n,7)) return 0;
    if (!wyhash_sprp(n,11)) return 0;
    if (!wyhash_sprp(n,13)) return 0;
    if (!wyhash_sprp(n,17)) return 0;
    if (!wyhash_sprp(n,19)) return 0;
    if (!wyhash_sprp(n,23)) return 0;
    if (!wyhash_sprp(n,29)) return 0;
    if (!wyhash_sprp(n,31)) return 0;
    if (!wyhash_sprp(n,37)) return 0;
    return 1;
}
//make your own secret
static inline void wyhash_make_secret(uint64_t seed, uint64_t *secret){
  ...
  for(size_t i=0;i<4;i++){
    ...
    do{
      ...
      if(ok&&!wyhash_is_prime(secret[i]))	ok=0;
    }while(!ok);
  }
}
